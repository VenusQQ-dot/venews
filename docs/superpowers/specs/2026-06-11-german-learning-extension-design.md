...
- 2026-07-06:新增發音功能 — schema 加入 `pronunciation`(IPA,AI 生成),
  解釋卡片與單字表 popup 加入 🔊 朗讀按鈕(`speechSynthesis`,`de-DE` 語音,免 API 費用)。
- 2026-07-14:新增間隔重複複習(類 Anki)—
  - `srs.js`:SM-2 變體演算法(四鍵評分 again/hard/good/easy、ease 係數、
    間隔天數、lapse 追蹤),popup 與複習頁共用。
  - `review.html/js`:全頁複習介面。正面顯示德文單字(不含冠詞,測驗性別記憶)
    並自動朗讀;翻面顯示完整解釋;按鈕附下次間隔預覽;
    鍵盤操作(空白鍵翻面、1–4 評分、S 朗讀)。「再一次」的卡片本次複習內重出。
  - 單字 entry 新增 `srs` 欄位 `{reps, lapses, ease, interval, due}`;
    無此欄位的舊單字視為新卡,免資料遷移。
  - popup 新增「📚 複習 (N)」按鈕顯示到期數,開新分頁進入複習。

## 不做的事(YAGNI)

- 不做帳號系統 / 雲端同步(本機儲存已滿足需求)。
- 不做 Firefox 相容層。
- 不做反向卡(中文→德文)、每日新卡上限 — 可作為日後迭代。
